df_gen_kimi <- read.csv(file.choose(), header = TRUE, sep = ",")
names(df_gen_kimi) <- gsub(" ", "_", names(df_gen_kimi))

post_n_missing <- sapply(df_gen_kimi[numeric_vars], function(x) sum(is.na(x)))
post_missing_df_gen_kimi <- data.frame(var = names(post_n_missing), n_missing = as.integer(post_n_missing))
print(post_missing_df_gen_kimi)
View(df_gen_kimi)

#ricostruzione sc
df_gen_kimi$happiness_score_sc <- as.numeric(scale(df_gen_kimi$happiness_score))
df_gen_kimi$log_gdp_per_capita_sc <- as.numeric(scale(df_gen_kimi$log_gdp_per_capita))
df_gen_kimi$social_support_sc <- as.numeric(scale(df_gen_kimi$social_support))
df_gen_kimi$healthy_life_expectancy_at_birth_sc <- as.numeric(scale(df_gen_kimi$healthy_life_expectancy_at_birth))
df_gen_kimi$freedom_to_make_life_choices_sc <- as.numeric(scale(df_gen_kimi$freedom_to_make_life_choices))
df_gen_kimi$generosity_sc <- as.numeric(scale(df_gen_kimi$generosity))
df_gen_kimi$perceptions_of_corruption_sc <- as.numeric(scale(df_gen_kimi$perceptions_of_corruption))
df_gen_kimi$positive_affect_sc <- as.numeric(scale(df_gen_kimi$positive_affect))
df_gen_kimi$negative_affect_sc <- as.numeric(scale(df_gen_kimi$negative_affect))


sapply(df_gen_kimi[c(numeric_vars)], function(x) sum(is.na(x)))



#Statistica descrittiva
# Seleziona solo le variabili numeriche
num_df_gen_kimi <- df_gen_kimi[sapply(df_gen_kimi, is.numeric)]

# Calcolo delle statistiche descrittive:
# Media, deviazione standard, minimo e massimo per ciascuna variabile numerica
descrittive <- data.frame(
  Variabile = names(num_df_gen_kimi),
  Media = sapply(num_df_gen_kimi, mean, na.rm = TRUE),
  Deviazione_Standard = sapply(num_df_gen_kimi, sd, na.rm = TRUE),
  Minimo = sapply(num_df_gen_kimi, min, na.rm = TRUE),
  Massimo = sapply(num_df_gen_kimi, max, na.rm = TRUE)
...
happiness_summary_gen <- aggregate(happiness_score ~ country, data = df_gen_kimi,
...
na_mat <- is.na(df_gen_kimi[, numeric_vars])

missing_by_country_initial <- aggregate(na_mat,
                                        by=list(country=df_gen_kimi$country),
                                        FUN=sum)
...
media_annuale_kimi_gen <- aggregate(happiness_score ~ year, df_gen_kimi, mean)

ts_media_gen <- ts(media_annuale_kimi_gen$happiness_score,
                   start = min(media_annuale_kimi_gen$year),
                   end   = max(media_annuale_kimi_gen$year),
                   frequency = 1)
...
trend <- lm(media_annuale_kimi_gen$happiness_score ~ media_annuale_kimi_gen$year)
summary(trend)
...
# Scatterplot (2x4)
plot_scatter_gen(df_gen_kimi, vars)

# Grafici dei residui (2x4)
plot_residuals_gen(df_gen_kimi, vars)
...
media_annuale_kimi <- aggregate(
  df_gen_kimi[, vars_sc],
  by = list(year = df_gen_kimi$year),
  FUN = mean,
  na.rm = TRUE
)


plot(media_annuale_kimi$year,
     media_annuale_kimi$happiness_score_sc,
     type = "o",
     pch = 16,
     lwd = 2,
     col = "darkgreen",
     ylim = range(media_annuale_kimi[, vars_sc]),
     xlab = "Anno",
     ylab = "Valori medi (standardizzati)",
     main = "Serie temporali delle variabili (2005–2022) kimi")

lines(media_annuale_kimi$year, media_annuale_kimi$log_gdp_per_capita_sc,
      type = "o", pch = 16, col = "blue")

lines(media_annuale_kimi$year, media_annuale_kimi$social_support_sc,
      type = "o", pch = 16, col = "orange")

lines(media_annuale_kimi$year, media_annuale_kimi$generosity_sc,
      type = "o", pch = 16, col = "purple")

lines(media_annuale_kimi$year, media_annuale_kimi$positive_affect_sc,
      type = "o", pch = 16, col = "red")

lines(media_annuale_kimi$year, media_annuale_kimi$negative_affect_sc,
      type = "o", pch = 16, col = "brown")

lines(media_annuale_kimi$year, media_annuale_kimi$freedom_to_make_life_choices_sc,
      type = "o", pch = 16, col = "darkolivegreen")

lines(media_annuale_kimi$year, media_annuale_kimi$healthy_life_expectancy_at_birth_sc,
      type = "o", pch = 16, col = "darkcyan")

lines(media_annuale_kimi$year, media_annuale_kimi$perceptions_of_corruption_sc,
