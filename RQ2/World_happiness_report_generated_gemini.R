...
# Variabili numeriche
numeric_vars <- names(df_gen_gemini)[sapply(df_gen_gemini, is.numeric)]
numeric_vars

# Variabili categoriche (fattori o caratteri)
categorical_vars <- names(df_gen_gemini)[sapply(df_gen_gemini, function(x) is.factor(x) || is.character(x))]
categorical_vars
...
print(post_missing_df_gen_gemini)
View(df_gen_gemini)
...
df_gen_gemini$freedom_to_make_life_choices_sc <- as.numeric(scale(df_gen_gemini$freedom_to_make_life_choices))
df_gen_gemini$generosity_sc <- as.numeric(scale(df_gen_gemini$generosity))
df_gen_gemini$perceptions_of_corruption_sc <- as.numeric(scale(df_gen_gemini$perceptions_of_corruption))
df_gen_gemini$positive_affect_sc <- as.numeric(scale(df_gen_gemini$positive_affect))
df_gen_gemini$negative_affect_sc <- as.numeric(scale(df_gen_gemini$negative_affect))

sapply(df_gen_gemini[c(numeric_vars)], function(x) sum(is.na(x)))


#Statistica descrittiva
# Seleziona solo le variabili numeriche
num_df_gen_gemini <- df_gen_gemini[sapply(df_gen_gemini, is.numeric)]

# Calcolo delle statistiche descrittive:
# Media, deviazione standard, minimo e massimo per ciascuna variabile numerica
descrittive <- data.frame(
  Variabile = names(num_df_gen_gemini),
  Media = sapply(num_df_gen_gemini, mean, na.rm = TRUE),
  Deviazione_Standard = sapply(num_df_gen_gemini, sd, na.rm = TRUE),
  Minimo = sapply(num_df_gen_gemini, min, na.rm = TRUE),
  Massimo = sapply(num_df_gen_gemini, max, na.rm = TRUE)
...
na_mat <- is.na(df_gen_gemini[, numeric_vars])

missing_by_country_initial <- aggregate(na_mat,
                                        by=list(country=df_gen_gemini$country),
                                        FUN=sum)
...
# Scatterplot (2x4)
plot_scatter_gen(df_gen_gemini, vars, var_labels)

# Grafici dei residui (2x4)
plot_residuals_gen(df_gen_gemini, vars, var_labels)
